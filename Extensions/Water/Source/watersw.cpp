...
namespace Horde3DWater
{
	...
	const char *fsWaterSWDebugView =
		"varying vec4 color;\n"
		"void main() {\n"
		"	gl_FragColor = color;\n"
		"}\n";

	ShaderCombination WaterNodeSW::debugViewShader;

	WaterNodeSW::WaterNodeSW( const WaterNodeTpl &waterTpl ) : WaterNode( waterTpl ),
		_vertexBuffer(0), _indexBuffer(0), vertices(NULL)
	{
	...
	void WaterNodeSW::createBuffers()
	{
	...
	void WaterNodeSW::destroyBuffers()
	{
		Modules::renderer().unloadBuffers( _vertexBuffer, _indexBuffer );
	}
	void WaterNodeSW::updateBuffers( float x1, float y1, float x2, float y2 )
	{
		float width = x2 - x1;
		float height = y2 - y1;
	...
	WaterNodeSW::~WaterNodeSW()
	{
		destroyBuffers();
		delete[] vertices;
	}

	bool WaterNodeSW::setParami( int param, int value )
	{
		Resource *res;
		switch( param )
		{
